...
model, answer, history = (
    faster_whisper.WhisperModel(
        model_size_or_path="base", device="cuda" if torch.cuda.is_available() else "cpu"
    ),
    "",
    [],
)
...
while True:
    ...
    audio_stream.stop_stream(), audio_stream.close(), audio.terminate()
    ...
    stream(client.generate(text=generator, voice=Voice(voice_id='FA6HhUjVbervLw2rNl8M'), model="eleven_multilingual_v2", stream=True))
